#!/usr/bin/env python3
"""
WAV folder -> FastTracker II Extended Instrument (.xi)

Interactive CLI version - just run: python script.py

Supports various naming patterns:
- INIT_C4_060.wav
- CHORD - Such 90s_A#0_022.wav
- TSP_SDG_chord_legato_C4_060.wav
- T_MKPLFS_SERUM_Lead_Dark_Square_C7_096.wav
- VOCAL - Hurts So Good_C#9_121.wav
...
- Bulk processing (recursively finds ALL folders with WAV files at any depth)

Fixes:
- Correct XI 1.2 header layout (num_samples at 0x128).
- **CRITICAL**: Length/loop fields are BYTES (not 16-bit words) for 16-bit samples.
- Robust WAV reader: PCM 8/16/24/32, IEEE float 32/64, WAVE_FORMAT_EXTENSIBLE.
- Proper stereo->mono averaging; keep full length by default (no silent truncation).
- EXACT Rel.Note + Finetune (matches your working .xi):
    * first sample:              Rel = 83 - MIDI,  Finetune =   0
    * middle samples:            Rel = 88 - MIDI,  Finetune = 104
    * last sample if MIDI > 60:  Rel = 28,         Finetune = 104
- Note→sample map = largest sampled MIDI ≤ played note (same as before).
"""
...
# Regex to match: ANYTHING_NOTE+OCTAVE_MIDI
...
║    ╚══╝╚══╝ ╚═╝  ╚═╝  ╚═══╝         ╚═╝    ╚═════╝     ╚═╝  ╚═╝╚═╝ ║
║                                                                      ║
║            FastTracker II Extended Instrument Converter             ║
║                                                                      ║
╚══════════════════════════════════════════════════════════════════════╝
"""
    ...
    def scan_directory(path: Path):
        ...
        try:
            for item in path.iterdir():
                if item.is_dir():
                    # Check if this directory has WAV files
...
def validate_output_path(path_str: str) -> Optional[Path]:
    ...
    # Add .xi extension if not present
...
# ---------------- WAV LOADER (PCM & FLOAT, incl. WAVE_FORMAT_EXTENSIBLE) ----------------
...
def _read_chunks(fp):
    ...
    while True:
        ...
        fp.seek(start + size + (size & 1))  # word-align
    return chunks
...
def load_wav_any(path: str):
    ...
    return _to_mono_int16(raw, fmt)  # array('h')
...
class Sample:
    def __init__(self, note: str, octave: int, midi: int, path: str):
        ...
        self.sample_type = 0x10  # 16-bit
        self.volume, self.panning = 64, 128
        self.finetune, self.relative_note = 0, 0

        self.data = b''                 # packed bytes (delta-encoded)
        self.length_bytes = 0           # *** BYTES ***
        self.loop_start_bytes = 0       # *** BYTES ***
        self.loop_length_bytes = 0      # *** BYTES ***

    def load(self, max_frames: int = 0):
        smp = load_wav_any(self.path)   # array('h') mono int16
        if max_frames and len(smp) > max_frames:
            smp = smp[:max_frames]
        if len(smp) & 1:                # keep even count for 16-bit packing
            smp = smp[:-1]
        ...
        # *** IMPORTANT: header wants BYTES for 16-bit samples ***
    ...
    def header_bytes(self) -> bytes:
        ...
        b = bytearray()
        b += struct.pack('<I', self.length_bytes)      # BYTES
        b += struct.pack('<I', self.loop_start_bytes)  # BYTES
        b += struct.pack('<I', self.loop_length_bytes) # BYTES
        b += struct.pack('B', self.volume)
        b += struct.pack('b', self.finetune)
        b += struct.pack('B', self.sample_type)        # 0x10 = 16-bit
...
def write_xi(samples: List[Sample], inst_name: str, out_path: str):
    ...
    out = bytearray()
    # ---- 66-byte instrument header ----
    out += b'Extended Instrument: '                                       # 21
    out += inst_name[:22].ljust(22).encode('ascii', 'replace')            # 22
    out += b'\x1A'                                                        # 1
    out += b'wav2xi len-bytes fix'[:20].ljust(20, b' ')                   # 20
    out += struct.pack('<H', 0x0102)                                      # 2  (v1.2)

    # Keymap (96)
    out += bytes(note_map)

    # Volume envelope points (12 pairs)
    vol_points = [(0,64),(64,64)]
    for i in range(12):
        x,y = vol_points[i] if i < len(vol_points) else (0,0)
        out += struct.pack('<HH', x, y)

    # Panning envelope points (12 pairs)
    pan_points = [(0,32),(64,32)]
    for i in range(12):
        x,y = pan_points[i] if i < len(pan_points) else (0,0)
        out += struct.pack('<HH', x, y)

    # 6 bytes vibrato+fade, 10 bytes env counts/flags, 22 reserved  -> num_samples at 0x128
    out += struct.pack('<BBBBH', 0,0,0,0, 0)                              # 6
    out += struct.pack('<BBBBBBBBBB',                                     # 10
...
def process_folder(input_dir: Path, output_path: Path, inst_name: str, show_progress: bool = True) -> Tuple[bool, int, int, str]:
    ...
    for i, p in enumerate(wavs, 1):
        try:
            ...
            s.load(0)  # max_length = 0 (keep full)
            samples.append(s)
            if show_progress:
...
def interactive_mode():
    ...
    if has_direct_wavs and all_wav_folders:
        ...
        # Show folders with their relative paths
        for folder in all_wav_folders[:10]:  # Show first 10
        ...
        if len(all_wav_folders) == 1:
            ...
            if choice == 0:
                process_mode = "single"
                # Let user pick which subdirectory
                clear_screen()
                show_header()
...
def process_single_folder(input_dir: Path) -> int:
    ...
    # Clear and show header for processing
    clear_screen()
    show_header()
    print(f"Input folder: {input_dir}")
    print(f"Output file: {output_path}")
...
def process_bulk_folders(base_dir: Path, all_folders: List[Path]) -> int:
    ...
    output_dir = base_dir  # default to same directory
    ...
    for i, subdir in enumerate(all_folders, 1):
        ...
        # Create output path - use the folder name for the .xi file
...
def main():
    """Main entry point"""
    # Check if any arguments were provided (old CLI mode for backwards compatibility)
    if len(sys.argv) > 1:
        import argparse
